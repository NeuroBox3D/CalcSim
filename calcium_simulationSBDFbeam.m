function calcium_simulationSBDF5Hzbeam(filename,outfolder,datafolder,load_syn,vtk)
%% This is the default input morphology
addpath(genpath([pwd, filesep, 'codes' ]));
if (nargin == 0)
    filename = 'morphos/beam64um/beamref5.swc';
    outfolder = 'vtk0_beamcalb40_5Hz';
    datafolder = 'results/beamcalb40_5Hz';
    load_syn = 0;
    vtk = 0;
end
...
vec1 = zeros(n,nT/save_every);
vec2 = zeros(n,nT/save_every);
vec3 = zeros(n,nT/save_every);
vec4 = zeros(n,nT/save_every);
vec5 = zeros(n,nT/save_every);
vec6 = zeros(n,nT/save_every);
vec7 = zeros(n,nT/save_every);
vec8 = zeros(n,nT/save_every);
vec9 = zeros(n,nT/save_every);
vec10 = zeros(n,nT/save_every);

rec_ind = [1:n]; %brchLst measuring at the branch points
...
for i=1:nT
    ...
    if (mod(i,save_every)==0) 
          ...
          vec8(:,i/save_every) = INa;
          vec9(:,i/save_every) = Ik;
          vec10(:,i/save_every) = Ic;
    end
    ...
    [Fv,INa,Ik,Ic] = cable_reaction(v,nn,mm,hh,qq,u(cci),i);
...
save(sprintf('%s/Na_%i.mat',datafolder,load_syn),'vec8');
save(sprintf('%s/k_%i.mat',datafolder,load_syn),'vec9');
save(sprintf('%s/Ca_%i.mat',datafolder,load_syn),'vec10');
end
...
function [yout,INa,Ik,Ic] = cable_reaction(V,n,m,h,q,cc,i)
